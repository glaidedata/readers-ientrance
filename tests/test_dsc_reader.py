...
def mock_dsc_file(tmp_path):
    """Creates a temporary mock PerkinElmer DSC .txt file for testing."""
    content = """Filename:	c:\\users\\sanchirico\\desktop\\gianluca\\dna_2_5_h_1.ds8d
Operator ID:
Sample ID:
...
Fine Balance: 0
	...
	DSC8500 SAMPLE TEMPERATURE CALIBRATION VALUES:
Date: 23/06/2025 17:53:28
Reference	Expected (°C)	Measured (°C)	Weight (mg)	Scan rate (°C/min)	Method
...
def test_read_perkinelmer_dsc(mock_dsc_file):
    ...
    # 8. Validate Bottom Metadata / Footer (Updated Keys!)
    assert dsc_data.metadata.get("MANUAL TUNE CALIBRATION VALUES_Date") == "27/10/2025 10:06:34"
    assert dsc_data.metadata.get("MANUAL TUNE CALIBRATION VALUES_Slope") == "61"

    # Verify the specific Firmware colon/tab fix is working
    assert "Build 4" in dsc_data.metadata.get("PROFILE VALUES FOR THIS DATA_Firmware Version")

    # 9. Validate Table Extraction Logic
    assert "SAMPLE TEMPERATURE CALIBRATION VALUES" in dsc_data.tables
    st_table = dsc_data.tables["SAMPLE TEMPERATURE CALIBRATION VALUES"]
    assert len(st_table) == 2  # Header row + Indium row
    assert st_table[1][0] == "Indium"
    assert st_table[1][1] == "156.600"