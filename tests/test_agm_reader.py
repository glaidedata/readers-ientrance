...
import numpy as np
import pandas as pd
from readers_ientrance import read_micromag_agm, AGMData
...
def test_read_micromag_agm_metadata():
    ...
    assert isinstance(result, AGMData)

    # Check the custom extracted headers
    # (Note: result.metadata is still a dictionary, so we use [] for the keys inside it)
...
def test_read_micromag_agm_data_arrays():
    ...
    # Check array lengths using dot notation (assuming you put 3 data rows in your dummy file)
