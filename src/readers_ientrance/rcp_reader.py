...
def _decode_ole_stream(data: bytes) -> Any:
    """Decodes OLE binary streams by prioritizing Numbers over Strings."""
    if not data:
        return None

    length = len(data)

    # 1. Number Decoding: 4-Byte (Int32 or Float32)
    # MUST BE BEFORE STRING DECODING to prevent false-positive Unicode translations
    if length == 4:
    ...
    # 3. Attempt String Decoding (Windows OLE strings are null-terminated)
    try:
